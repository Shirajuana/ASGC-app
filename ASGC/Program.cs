...
namespace ASGC
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("Enter student assignment scores: ");
        ...
        {
            double avgAssignments = assignments.Count > 0 ? (double)assignments.Sum() / assignments.Count : 0;
            double avgQuizzes = quizzes.Count > 0 ? (double)quizzes.Sum() / quizzes.Count : 0;
        ...
        {
            double weightedAverage = GradeCalc(assignments, quizzes, finalExam, ass_w, quiz_w, final_w);
